        ...
        m_broker(broker)
{
    ...
    RegisterSubhandle("lb.DisableMig",boost::bind(&LBAgent::HandleDisableMig, this, _1, _2));
    ...
    Kei = P_Migrate;
    //initialize flag for power migration
    m_makeMig = true;
}
...
void LBAgent::HandleDisableMig(CMessage msg, PeerNodePtr peer)
{
    Logger.Trace << __PRETTY_FUNCTION__ << std::endl;
    m_makeMig = false;
}
...
void LBAgent::HandleCollectedState(CMessage msg, PeerNodePtr peer)
{
    ...
    //current, GrossP is only this leader's power
    typedef device::CDeviceSst SST;
    m_Gateway = m_phyDevManager->GetValue<SST>(&SST::GetGateway,
                std::plus<device::SettingValue>());
    GrossP = m_Gateway*m_Gateway;
    ...
    if (m_invFlag == true)
    {
        ...
        Logger.Status << "----------GrossP is " << GrossP << "-----------------" << std::endl;
        ...
        double right = Kei*(w-376.8);
        Logger.Status << "Physical Invariant Formular : left is " << left
        << "  right is " << right;
        
        //set guard for power migration
        if (left < right)
        {
            m_makeMig = false;
            CMessage m_;
            m_.m_submessages.put("lb.source", GetUUID());
            m_.SetHandler("lb.DisableMig");
            BOOST_FOREACH(PeerNodePtr peer, m_AllPeers | boost::adaptors::map_values)
            {
                if (peer->GetUUID() != GetUUID())
                {
